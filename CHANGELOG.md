...
- **Qobuz downloads honor the app's settings in full parity with Deezer.** Quality now follows the Quality setting exactly (a format-key mismatch made every Qobuz download silently fall back to MP3 320 even with FLAC selected); Bitrate Fallback is enforced (FLAC-unavailable tracks error instead of silently downgrading when fallback is off, and a downgraded file gets the correct .mp3 extension plus the "Lower bitrate" indicator); playlist downloads use the playlist folder, playlist track template, and compilation tagging; separate cover files (cover.jpg) respect the artwork settings; embedded artwork respects the embed toggle; duplicate-track skip (by ISRC) and the overwrite mode (skip/overwrite/rename) apply; and the full tagging option set (null separator, ID3v1, main-artist-only, Various Artists handling, artist combinations) carries over. Qobuz has no lyrics API, so lyrics settings don't apply to Qobuz downloads.
...
- **Readability bump on core reading type.** Sidebar nav labels and track titles 13 → 14.5 px, track artist line 11.5 → 12.5 px, mono index/duration 11 → 12 px; Transfer Rack units follow suit — artist line 9.5 → 11.5 px, track count/percentage 9.5 → 11 px (VU meter and titles unchanged). Same weights, colors, and spacing; deliberate mono micro-labels are unchanged.
