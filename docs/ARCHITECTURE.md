...
- **Design system (2.0 "Signal Deck"):** theming is a CSS-variable system in `src/assets/main.css` — each theme block under `[data-theme="..."]` defines RGB-triple variables (`--bg-main`, `--primary-500`, `--logo-*`, …) that `tailwind.config.js` consumes via `rgb(var(--…))`, so Tailwind utility classes are theme-agnostic and switching theme is one attribute flip on `<html>`. `[data-mode="light"]` blocks override per theme (Signal gets a dedicated "paper console" olive variant because chartreuse fails contrast on paper). Fonts (Archivo Black display, IBM Plex Sans/Mono) are bundled via `@fontsource` imports in `src/main.ts` — no network fetch, fully offline. The Signal-only scanline/vignette atmosphere is two pointer-transparent divs in `App.vue` (`.theme-crt`/`.theme-vig`) whose visibility is gated by theme in CSS, off in light mode.
