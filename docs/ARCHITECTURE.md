...
Concurrency and pacing: `processQueue()` drains the queue up to `maxConcurrentDownloads` in parallel. An optional **download pacing** gate (issue #86, `setPacing(true)`) sits in front of new starts — when enabled it holds each new download until a jittered minimum gap has elapsed since the last start, so a large queue trickles out instead of bursting (which can trip Deezer's "unusual activity" detection). It's off by default and a true no-op when off; it throttles only the *start rate*, never concurrency. Because Playlist Sync and Artist Sync enqueue through this same `download()` → `processQueue()` path, pacing covers sync runs automatically.
