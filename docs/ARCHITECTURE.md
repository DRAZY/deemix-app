...
Concurrency and pacing: `processQueue()` drains the queue up to `maxConcurrentDownloads` in parallel. An optional **download pacing** gate (issue #86, `setPacing('off' | 'balanced' | 'cautious')`) sits in front of new starts — on `balanced`/`cautious` it holds each new download until a jittered minimum gap has elapsed since the last start (base ~2.5s / ~7s respectively, ±50% jitter), so a large queue trickles out instead of bursting (which can trip Deezer's "unusual activity" detection). It's `off` by default and a true no-op when off; it throttles only the *start rate*, never concurrency. Because Playlist Sync and Artist Sync enqueue through this same `download()` → `processQueue()` path, pacing covers sync runs automatically.
